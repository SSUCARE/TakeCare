...
import com.ssu.takecare.assist.graph.MyMarkerView;
...
public class WeightFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        make_graph(view.findViewById(R.id.linechart));

        return view;
    }

    /* LineChart 만들기 */
    public void make_graph(LineChart lineChart){
        // 차트 초기화 작업
        ...
        int YLabelMin=50;
        if(only_weight_list.size()>0){
            YLabelMin=0;
            for(int i=0; i<only_weight_list.size(); i++){
                YLabelMin+=only_weight_list.get(i);
            }

            YLabelMin/=only_weight_list.size();
            if(YLabelMin%2==0){
                YLabelMin-=14;
            }
            else{
        ...
        yAxisLeft.setAxisMaximum(YLabelMin+30);
